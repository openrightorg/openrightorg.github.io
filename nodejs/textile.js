...
function convert(t) {
	...
	for(i=0;i<lines.length;i++) {
		...
		if ((lines[i].indexOf("|") == 0)  || (lines[i].match(trstyle)) ) {
			stp(1);
			if(!intable) {html += '<table style="border-width: 1px; border-style: solid; border-spacing: 0; border-collapse: collapse;">\n';intable=1;}
			var rowst="";var trow="";
			...
			for(j=1;j<cells.length-1;j++) {
				...
				var al=/^([<>=^~\/\\\{]+.*?)\.(.*)/.exec(cells[j]);
				var at="",st="border-width: 1px; border-style: solid;";
				if(al != null) {
					cells[j]=al[2];
...
function prep(m){
	...
	for(i in tags) {
		m = make_tag(m,RegExp("^"+tags[i]+"([^\\s].+?[^\\s])"+tags[i]),i,"");
		m = make_tag(m,RegExp("\\s"+tags[i]+"([^\\s].+?[^\\s])"+tags[i]),i," ");
	}
	...
	m=m.replace(/\"([^\"]+)\":([A-Z][a-z]+[A-Z][a-z]\w*)/g,'<a href="$2' + document.location.pathname.replace(/.*?(\.[^\.]*|)$/, function(m,s){return s}) + '">$1<\/a>');
	...
	m=m.replace(/\[\[([^\]\|]+)\|([^\]]+)\]\]/g,'<a href="$2' + document.location.pathname.replace(/.*?(\.[^\.]*|)$/, function(m,s){return s}) + '">$1<\/a>');
	m=m.replace(/\[\[([^\]\|\.]+)\]\]/g,'<a href="$1' + document.location.pathname.replace(/.*?(\.[^\.]*|)$/, function(m,s){return s}) + '">$1<\/a>');
	return m;
}
...
function convert1(text) {
        return text.replace(/([\s\S]*?)(?:<verbatim>([\s\S]*?)<\/verbatim>|bc\.\s((?:[ \t]*\S.*(?:\n|$))*)|<pre><code>([\s\S]*?)<\/code><\/pre>|$)/g,
                function(m,t,c,c2,c3) {
			if(c === undefined) { c = c2; }
			if(c === undefined) { c = c3; }
                        return convert(t) +
                                (c !== undefined ?
...
window.onload = function() {
    ...
    text = text.replace(/^\s*<pre>([\s\S]*)<\/pre>\s*$/ig, "$1");
    // snc correction. '../files' online means 'attachments/' in the repo.
    text = text.replace(/(\!)(?:\.\.\/)?files\/([^\!]+!)/g, function(m,p,s) { return p + 'attachments/' + s});
    text = text.replace(/(src=\")(?:\.\.\/)?files\/([^\"]+\")/g, function(m,p,s) { return p + 'attachments/' + s});
    text = text.replace(/("[^"]*?":)(?:\.\.\/)?files\//g, function(m,f) { return f + 'attachments/'});
    document.body.innerHTML = convert1(text);
};
